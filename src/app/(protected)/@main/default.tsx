...
export default async function CalendarPage({
  ...
  const blocks: OccupancyBlock[] = [
    ...rangeBookings.map((b) => ({
      ...
      tooltipHeading: b.vesselName,
      tooltipLines: [
        b.startDate === b.endDate ? b.startDate : `${b.startDate} – ${b.endDate}`,
        `Status: ${b.status}${b.overridden ? " (overridden)" : ""}`,
        `Booked by ${formatStaffName(b.staffName, b.staffTitle, b.staffRole)}`,
      ],
    ...
    ...rangeEvents.map((e) => ({
      ...
      tooltipHeading: e.name,
      tooltipLines: [
        e.startDate === e.endDate ? e.startDate : `${e.startDate} – ${e.endDate}`,
        `Event${e.overridden ? " (overridden)" : ""}`,
        `Created by ${formatStaffName(e.staffName, e.staffTitle, e.staffRole)}`,
      ],
    ...
    ...rangeClosures.map((c) => ({
      ...
      tooltipHeading: "Closed",
      tooltipLines: [
        c.startDate === c.endDate ? c.startDate : `${c.startDate} – ${c.endDate}`,
        `${c.reason}${c.overridden ? " (overridden)" : ""}`,
        `Created by ${formatStaffName(c.staffName, c.staffTitle, c.staffRole)}`,
      ],
